...
function decrStock(items){const ps=DB.get('bmb_products_v2',[]);const changed=[];items.forEach(it=>{const p=ps.find(x=>x.id===it.id);if(!p)return;const c=(p.colors||[]).find(x=>x.name===it.color);if(c&&c.sizes[it.size]!=null){c.sizes[it.size]=Math.max(0,(+c.sizes[it.size])-it.qty);changed.push(p)}});DB.set('bmb_products_v2',ps);renderShop();renderTrend();try{if(typeof Cloud!=='undefined'&&Cloud.on())changed.forEach(p=>Cloud.saveProduct(p).catch(()=>{try{Cloud.queue('product',p)}catch(x){}}))}catch(e){}}
