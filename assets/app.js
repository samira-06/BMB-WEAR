...
const gal=L.length?L.map((u,i)=>`<div class="${i===0?'on':''}" onclick="galGo(${i},event)"><img src="${u}" onerror="this.parentElement.remove()"></div>`).join(''):`<div class="on">${p.emoji||'👕'}</div>`;
...
<div id="pd_gal"><div class="gal-main" id="galMain">${L[0]?`<img src="${L[0]}" onerror="this.remove()">`:(p.emoji||'👕')}</div><div class="gal-th">${gal}</div></div>`}
