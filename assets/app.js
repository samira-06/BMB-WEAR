...
return `<a class="coll" href="boutique.html?cat=${c.cat}"><div class="bg">${img?`<img src="${img}" alt="">`:c.tag[0]}</div><div class="tx"><span class="tag">${c.tag}</span><h3>${p?p.name:c.t}</h3><span class="coll-price">${p?p.price.toLocaleString()+' FCFA':c.d}</span><span class="btn">Shopper</span></div></a>`}).join('')}
...
function sum(){const os=$('osum');if(!os)return;const nom=$('c_nom')?.value.trim(),tel=$('c_tel')?.value.trim(),qv=$('c_quart')?.value||'';
...
else{const f=DB.get('bmb_favs_'+u.tel,[]);$('acc_box').innerHTML=`<div class="panel"><h3>Salut ${u.name} <button onclick="logout()">Sortir</button></h3><p><small>${u.tel} • ${u.email} • ${u.addr||''}</small></p></div><div class="track-grid" style="margin-top:1rem"><div class="panel"><h3>Favoris</h3><div class="fav-grid">${f.map(id=>{const p=prods.find(x=>x.id===id);if(!p)return '';const im=coverOf(p);return `<div class="fav-card" onclick="openP('${p.id}')">${im?`<img src="${im}" onerror="this.remove()">`:`<span style="font-size:2rem">${p.emoji||'👕'}</span>`}<b>${p.name}</b><small>${p.price.toLocaleString()} F</small></div>`}).join('')||'Aucun'}</div></div><div class="panel"><h3>Mes commandes</h3>${DB.get('bmb_orders',[]).filter(o=>o.tel===u.tel).slice(0,6).map(o=>`<p><b>${o.num}</b> ${o.total.toLocaleString()} — ${o.status}</p>`).join('')||'Aucune'}</div></div>`}}
...
updCart();renderAcc();buildZones();['c_nom','c_tel','c_adr'].forEach(id=>{const el=$(id);if(el&&!el.dataset.sum)el.dataset.sum='1',el.addEventListener('input',sum)});$('c_zone')?.addEventListener('change',sum);$('c_quart')?.addEventListener('change',syncZone);
