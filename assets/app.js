...
function renderCatsHome(){const el=$('catshome');if(!el)return;const ps=DB.get('bmb_products_v2',[]);
...
async function boot(){try{if(typeof Catalog!=='undefined'){const c=await Catalog.load();if(c){const r=Catalog.merge(DB.get('bmb_products_v2',[]),c);if(r.changed)DB.set('bmb_products_v2',r.list)}}}catch(e){}applyCatQuery();renderShop();renderTrend();renderColls();renderNew();renderBest();renderCatsHome();try{if(typeof Cloud!=='undefined'&&Cloud.on()){const ps=await Cloud.fetchProducts();if(ps&&ps.length){DB.set('bmb_products_v2',ps);renderShop();renderTrend();renderColls();renderNew();renderBest();renderCatsHome()}}}catch(e){}}
boot();
setInterval(async()=>{try{if(typeof Cloud!=='undefined'&&Cloud.on()){const ps=await Cloud.fetchProducts();if(ps&&ps.length&&JSON.stringify(ps)!==JSON.stringify(DB.get('bmb_products_v2',[]))){DB.set('bmb_products_v2',ps);renderShop();renderTrend();renderColls();renderNew();renderBest();renderCatsHome();toast('Nouveautés synchronisées')}}}catch(e){}},45000);
