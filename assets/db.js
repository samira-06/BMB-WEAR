...
async pushOrder(o){const b={num:o.num,name:o.nom,tel:o.tel,zone:o.zone||'',quartier:o.quartier||'',adr:o.adr||'',pay:o.pay||'',code:o.code||'',note:o.note||'',items:o.items||[],total:o.total||0,status:o.status||'En attente',deadline:o.deadline||0,hist:o.hist||[]};const post=x=>this.req('orders',{method:'POST',headers:this.H({'Prefer':'resolution=merge-duplicates'}),body:JSON.stringify(x)});try{await post(b)}catch(e){const m={num:o.num,name:o.nom,tel:o.tel,zone:o.zone||'',quartier:o.quartier||'',adr:o.adr||'',pay:o.pay||'',code:o.code||'',note:o.note||'',items:o.items||[],total:o.total||0,status:o.status||'En attente',deadline:o.deadline||0};await post(m)}},
async fetchOrders(){try{return (await this.req('orders?select=*&order=created_at.desc&limit=200')).map(o=>({num:o.num,nom:o.name,tel:o.tel,zone:o.zone,quartier:o.quartier,adr:o.adr,pay:o.pay,code:o.code,note:o.note,items:o.items,total:o.total,status:o.status,date:o.created_at,deadline:o.deadline,hist:o.hist||[]}))}catch(e){return[]}},
async fetchOrder(num){try{const r=await this.req('orders?num=eq.'+encodeURIComponent(num)+'&select=*&limit=1');if(!r||!r.length)return null;const o=r[0];return{num:o.num,nom:o.name,tel:o.tel,zone:o.zone,quartier:o.quartier,adr:o.adr,pay:o.pay,code:o.code,note:o.note,items:o.items,total:o.total,status:o.status,date:o.created_at,deadline:o.deadline,hist:o.hist||[]}}catch(e){return null}},
mergeOrders(cur,cloud){const pend=new Set();try{JSON.parse(localStorage.getItem('bmb_outbox')||'[]').forEach(o=>{if(o.t==='order'&&o.d&&o.d.num)pend.add(o.d.num)})}catch(e){}let ch=false;const out=(cur||[]).slice();
const norm=o=>({num:o.num,nom:o.nom,tel:o.tel,zone:o.zone||'',quartier:o.quartier||'',adr:o.adr||'',pay:o.pay||'',code:o.code||'',note:o.note||'',items:o.items||[],total:+o.total||0,status:o.status||'',deadline:o.deadline||0,hist:o.hist||[]});
...
const e0=devs[id]||{};e0.v=60;e0.seen=new Date().toISOString();if(ev==='push')e0.pushes=(e0.pushes||0)+1;if(ev==='fail')e0.fails=(e0.fails||0)+1;devs[id]=e0;
...
const id=this.devId();let devs={};try{const r=await this.req('settings?key=eq.diag&select=value&limit=1');if(r&&r[0]&&r[0].value)devs=r[0].value.devices||{}}catch(e){}
const e0=devs[id]||{};e0.v=60;e0.seen=new Date().toISOString();e0.err=String(m||'').slice(0,160);devs[id]=e0;
await this.req('settings',{method:'POST',headers:this.H({'Prefer':'resolution=merge-duplicates'}),body:JSON.stringify({key:'diag',value:{devices:devs}})})}catch(e){}},
